...
const CMA_ROOT = `https://api.contentful.com`;
...
async function getTokenOwnerId(): Promise<string> {
  ...
  const r = await fetch(`${CMA_ROOT}/users/me`, {
  ...
  try {
    ...
    const result = type === 'task'
      ...
      : await cmaPost(`/entries/${entryId}/comments`, { body: cmaBody },
          payload.fieldId ? { 'X-Contentful-Parent-Entity-Reference': `fields.${payload.fieldId}.${payload.locale || 'en-US'}` } : undefined);
